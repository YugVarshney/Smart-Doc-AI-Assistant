...
def get_embeddings(texts: List[str]) -> List[List[float]]:
    ...
    if settings.EMBEDDING_BACKEND == "openai":
        try:
            ...
            model_name = settings.EMBEDDING_MODEL or ("text-embedding-004" if "google" in (settings.OPENAI_BASE_URL or "").lower() else "text-embedding-3-small")
            ...
            for data in resp.data:
                ...
                if len(emb) != settings.EMBEDDING_DIM:
                    logger.warning(f"OpenAI embedding dimension mismatch: got {len(emb)}, expected {settings.EMBEDDING_DIM}. Adjusting vector size.")
                    if len(emb) > settings.EMBEDDING_DIM:
                        emb = emb[:settings.EMBEDDING_DIM]
                    else:
                        emb = emb + [0.0] * (settings.EMBEDDING_DIM - len(emb))
                    
                    # Re-normalize the adjusted vector
                    emb_arr = np.array(emb)
                    norm = np.linalg.norm(emb_arr)
                    if norm > 1e-9:
                        emb = (emb_arr / norm).tolist()
                embeddings.append(emb)
            return embeddings
        except Exception as e:
            logger.error(f"API embeddings creation failed: {e}")
            raise e
            
    model = _load_emb_model()
    embs = model.encode(texts, convert_to_numpy=True, show_progress_bar=False)
    
    # Ensure they match settings.EMBEDDING_DIM
    adjusted_embs = []
    for emb in embs:
        if len(emb) != settings.EMBEDDING_DIM:
            logger.warning(f"Local embedding dimension mismatch: got {len(emb)}, expected {settings.EMBEDDING_DIM}. Adjusting vector size.")
            if len(emb) > settings.EMBEDDING_DIM:
                emb = emb[:settings.EMBEDDING_DIM]
            else:
                emb = np.pad(emb, (0, settings.EMBEDDING_DIM - len(emb)), 'constant')
        adjusted_embs.append(emb)
        
    adjusted_embs = np.array(adjusted_embs)
    norms = np.linalg.norm(adjusted_embs, axis=1, keepdims=True)
    normalized_embs = adjusted_embs / (norms + 1e-12)
...
def index_document_in_db(doc_id: str, file_path: str):
